...
summ_hatch <- data_tstats %>%
  mutate(trt.win = case_when(trt.f == trt.m & trt.f == 260 ~ "26-26",
         ...
         trt.btwn = case_when(trt.type == "ctrl" ~ "26-26", 
                              (trt.type == "sex-f" | trt.type == "sex-m") & (trt.f == 419 | trt.m == 419) ~ "40-19",
                              (trt.type == "sex-f" | trt.type == "sex-m") & (trt.f == 426 | trt.m == 426) ~ "40-26",
                              (trt.type == "sex-f" | trt.type == "sex-m") & (trt.f == 433 | trt.m == 433) ~ "40-33",
                              #trt.type == "sex-m" ~ as.numeric(trt.m),
                              #trt.type == "sex-f" ~ as.numeric(trt.f)
                              )) %>%
  group_by(#id.tent,
           trt.f, trt.m, pop,
           trt.win, trt.btwn, trt.type
           ) %>%
...
summ_longevity %>%
  ...
  scale_color_manual(values = RYB,
                     labels = labels.wintrt,) +
  scale_x_discrete(labels = labels.wintrt) +
