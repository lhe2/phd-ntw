...
# load data, helper fns
```{r}
here::i_am("scripts/analysis/2023/analyses-F1.Rmd")
```
...
```{r}
source(here("scripts/analysis/2023/load-F1.R"))

dfs_F1 <- dfs_F1 %>%
  lapply(., \(x){
    x %>%
      mutate(across(starts_with(c("trt", "parent")), as.factor),
             pop = factor(pop, levels = c("parent", "F1")))
  })
```

# 260623 viz for lab mtg
```{r}
# prep fns
.calc3 <- function(x){
  x %>%
...
dfs_F1$wide %>%
  filter(pop == "F1") %>%
  group_by(across(c(starts_with(c("trt", "parent")), "pop", "tent"))) %>%
  .calc3() %>% #View()
  ggplot(aes(y = avg, x = trt.type,
             col = interaction(parent.minT, parent.trtsex),
             shape = interaction(parent.minT, parent.trtsex), 
             lty = interaction(parent.minT, parent.trtsex),
             group = tent)) +
  geom_line() +
    geom_point(size = 2) +
  geom_errorbar(aes(ymax = avg + se,
