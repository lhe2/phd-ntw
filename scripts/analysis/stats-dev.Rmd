...
## TODO
- separate tents & dev lol? (dfs_dev and devs_tents)?
...
```{r dev data prep}
# formatting (dont think this needs to be a list tbh LOL)
# for stats, drop ctrls and lab+TB
imported_dev <- 
  ...
  lapply(., \(x){
    x %>%
      ...
      filter(trt != 260) %>%
      mutate(across(c("year", "trt.minT"), as.factor),
...
dfs <- list(
  ...
  wide_pups =
    imported_dev[["wide"]] %>%
    filter(!is.na(jdate.pupa) & !is.na(sex))
...
mods_pupsurv <- list(
  glm_fullint = glm(is.pup ~ year*pop*trt.minT, 
...
mods_pupmass <- list(
  add = lm(mass.pupa ~ year + trt.minT + pop + sex,
           data = df, na.action = na.fail),
  fullint = lm(mass.pupa ~ year*trt.minT*pop*sex,
               data = df, na.action = na.fail)
...
mods_pupmass <- list_modify(
  ...
  twoway = lm(mass.pupa ~ (year + trt.minT + pop + sex)^2,
              data = df, na.action = na.fail),
  threeway = lm(mass.pupa ~ year*trt.minT*pop + sex,
              data = df, na.action = na.fail)
)
...
mods_puptime <- list(
  fullint = lm(1/tt.pupa ~ year*trt.minT*pop, data = dfs$wide)
  )
```
...
mods_pup_randyr <- list(
  ...
  surv_ctsyr = glmer(is.pup ~ pop*trt.minT + (1|year),
...
mods_pup_randyr <- list(
  time_lmertest = lmerTest::lmer(tt.pupa ~ trt.minT*pop + (1|year), data = dfs$wide),
  time_lme4 = lme4::lmer(tt.pupa ~ trt.minT*pop + (1|year), data = dfs$wide)
)
