...
merge adult/larval data in two steps (bc the col ctrls break `coalesce()` bc "missingness is uneven"):
...
```
standardising values + matching cols for merging: some cols do not mean the "same" thing across years:
...
dfs_tidy <- list_modify(
  ...
  long = dfs_tidy$wide %>% 
    mutate(tt.3rd = jdate.3rd - jdate.hatch,
           ...
           tt.long = case_when(is.na(is.fridge) ~ jdate.long - jdate.eclose),
           ...
           tt.fridge = case_when(!is.na(is.fridge) ~ jdate.tent - jdate.eclose)
