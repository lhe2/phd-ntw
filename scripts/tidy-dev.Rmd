...
dfs_stdised <- lapply(dfs_filtered, \(x){
  x %>%
  mutate(across(c("trt", "id", starts_with("jdate")), as.numeric))
})
```
...
# redo: survival, sup coding (counts after reaching 5th), temp trt info
# fix: exit dates
# add parsing of non-ntw temps
  ...
  x %>%
    mutate(is.pup = case_when(fate.dev %in% c("ec", "pup") ~ 1,
           ...
           is.sup = case_when(!is.na(sup) ~ 1,
                              TRUE ~ 0),
           meanT = case_when(trt %in% c(260, 267) ~ 26,
                             trt == 419 ~ 29.5,
                             trt %in% c(426, 330) ~ 33,
                             trt == 433 ~ 36.5),
           ...
           maxT = case_when(trt == 260 ~ 26,
                            trt %in% c(267, 330) ~ 33,
                            TRUE ~ 40),
           minT = case_when(trt %in% c(260, 426) ~ 26,
                            trt %in% c(419, 267) ~ 19,
                            trt %in% c(433, 330) ~ 33),
