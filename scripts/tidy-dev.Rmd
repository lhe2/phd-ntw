...
# redo: survival, pup/ec/sup coding (counts after reaching 5th)
# fix: exit dates; LPI info
# add parsing: non-ntw trts, expt vs control temp trts
dfs_subsets <- list(
  all = data.table::rbindlist(dfs_stdised, fill = TRUE) %>% 
    mutate(
...
# pre-subset for analyses/expts (mostly want the NTW one tho)

dfs_subsets$all <- dfs_subsets$all %>%
    ## calc dev times for pivot
...
dfs_subsets <- list_modify(
  ...
  ntw = dfs_subsets$all %>%
    ...
    select(-instar.enter),
  y23_temps = dfs_subsets$all %>%
    filter(year == 2023 & pop == "lab" & instar.enter == "hatch" &
             trt %in% c(260, 267, 330, 426)),
  y23_acc = dfs_subsets$all %>%
...
```{r}
# for now, just exporting NTW stuff...
dfs_tidy <- list(
  wide = dfs_subsets$ntw,
  long = dfs_subsets$ntw %>% 
...
rm(dfs_imported, dfs_filtered, dfs_stdised, dfs_subsets)
```
