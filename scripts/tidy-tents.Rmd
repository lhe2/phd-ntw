  ...
  x %>%
    mutate(mate.type = case_when(trt.f == trt.m ~ "within",
           ...
           mate.hs = case_when(trt.f > trt.m | is.na(trt.m) ~ "f",
                               ...
                               TRUE ~ "both"),
            ## "NA" if 260+260
           mate.ishs = case_when(mate.hs == "both" & mate.trt == 260 ~ NA_character_,
                                TRUE ~ mate.hs),
           trt.ishs = case_when(mate.trt == 260 ~ "ctrl",
                                TRUE ~ "expt"),
...
dfs_stdised = list(
  info = dfs_imported[["ad24"]] %>% 
    ...
    rename(pop.f = pop,
           trt.f = trt,
           id.f = id) %>%
    mutate(mate.type = "virgin f",
           mate.pop = pop.f,
           mate.trt = trt.f,
           mate.col = NA,
           # mate.hs = case_when(trt.f > 400 ~ "f"), 
           # mate.ishs = case_when(trt.f == 260 ~ NA_character_,
           #                       TRUE ~ "f"),
           mate.hs = "f",
           mate.ishs = "f",
           trt.ishs = case_when(trt.f > 400 ~ "expt",
                                TRUE ~ "ctrl"),
           is.hs.f = case_when(trt.f > 400 ~ 1,
                               TRUE ~ 0),
    ),
...
dfs_export <- list_modify(
  ...
  tents24 = dfs_stdised$events %>%
    merge(., dfs_stdised$info %>% 
            select(c("cage", starts_with(c("trt", "mate", "is")))) %>% distinct(), 
          by = c("cage")) %>%
    mutate(year = 2024,
           # missing .m stuff
           is.hs.m = NA, trt.m = NA,
