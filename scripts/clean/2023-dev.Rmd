...
here::i_am("_ntw/scripts/clean/2023-dev.Rmd")
library(here)
```
...
# larva data
dfs_raw <- list(
  field = read.csv(here("_ntw/data/gsheets/2023/dev-field.csv"), header = TRUE),
  
  labsu = read.csv(here("_ntw/data/gsheets/2023/dev-su.csv"), header = TRUE),
  
  labF1 = read.csv(here("_ntw/data/gsheets/2023/dev-F1.csv"), header = TRUE),
  
  labsp = read.csv(here("_ntw/data/gsheets/2023/dev-sp.csv"), header = TRUE)
)
```
...
df_cleaned <- df_cleaned %>%
  mutate(final.fate = case_when(!is.na(jdate.exit) & is.na(final.fate) ~ "pupa",
...
# raw data
write.csv(df_all, file = paste0(here("_ntw/data/raw/2023/"), "dev_", today, ".csv"), row.names = FALSE) 
...
# write.csv(export, file = paste0(here("_ntw/data/archive/2023/"), "dev-MQ_", today, ".csv" ), row.names = FALSE)
...
export <- df_cleaned %>%
  ...
  select(-c("from.E", "location", starts_with("h."), "time.in.approx", "dv", "pupa.deformities", "mass.died", "toss.if", "old.date.pmd", "reason.pmd", "fate.code", "notes", "mass.stuck", ends_with(".culled"), "eclose-3"))

write.csv(export, file = here("_ntw/data/clean/2023/dev.csv"), row.names = FALSE)
#write.csv(export, file = paste0(here("_ntw/data/archive/2023/"), "dev-sexta_", today, ".csv" ), row.names = FALSE)
```
