...
dfs_ldev <- list( #dfs_dev_ss
  ss_all = #dev_outcomes_all (incls pmd)
    dfs_tidy$long %>% 
    filter(pop != "col") %>%
  ...
  ss_ispup_unparsed = # dev_outcomes_allgrpd
    dfs_tidy$long %>%
    filter(pop != "col") %>%
    mutate(is.pup = case_when(is.pup == 2 ~ 1,
                              TRUE ~ as.numeric(is.pup))) %>%
    group_by(across(c(is.pup, 
                      year, pop, diet, starts_with("trt"), instar))) %>%
    .CalcDevSS(),
  ...
  # survivors/culled bugs kept separate
  ss_ispup_parsed = #dev_outcomes_allgrpd2
    dfs_tidy$long %>%
    filter(pop != "col") %>%
    # mutate(is.pup = case_when(is.pup == 2 ~ 1,
    #                           TRUE ~ as.numeric(is.pup))) %>%
...
dfs_adev <- list( #dfs_dev_ss
  ...
  ss_bysex = #ad_dev_bysex
    dfs_tidy$long %>%
    filter(instar %in% c("pupa", "eclose", "long"), !is.na(sex)) %>%
    group_by(across(c(year, pop, diet, starts_with("trt"), instar, sex))) %>%
    mutate(mass = mass/1000) %>%
    .CalcDevSS(),
  
    ss_all_iffr = #ad_dev_fr
    dfs_tidy$long %>%
    filter(instar %in% c("pupa", "eclose", "long")) %>%
    group_by(across(c(if.fridge,
                      year, pop, diet, starts_with("trt"), instar))) %>%
    mutate(mass = mass/1000) %>%
    .CalcDevSS()
...
```{r}
dfs_ldev <- list_modify( #dfs_dev_ss
  dfs_ldev,
  props_surv = #surv_outcomes
    dfs_tidy$wide %>%
    filter(is.pup != 2) %>%
