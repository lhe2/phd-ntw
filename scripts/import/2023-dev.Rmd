...
about:
- cleans ntw 2023 data (larval and tents) from googlesheets
...
here::i_am("scripts/import/2023-dev.Rmd")
library(here)
```
...
# larva data
dfs_raw <- list(
  field = read.csv(here("data/gsheets/2023/dev-field.csv"), header = TRUE, check.names = FALSE),
  
  labsu = read.csv(here("data/gsheets/2023/dev-su.csv"), header = TRUE, check.names = FALSE),
  
  labF1 = read.csv(here("data/gsheets/2023/dev-F1.csv"), header = TRUE, check.names = FALSE),
  
  labsp = read.csv(here("data/gsheets/2023/dev-sp.csv"), header = TRUE, check.names = FALSE)
)
```
...
df_all <- lapply(dfs_raw, \(x){
  x %>%
    ...
    mutate(mutate(across(starts_with("mass."), ~ as.numeric(.x))))
}) %>%
...
# individual fixes
df_filtered[df_filtered$id == "1415", "sex"]$sex <- "m" # typo?
  ## in all other sheets this is listed as an m...
```
...
df_cleaned <- df_filtered %>% 
  mutate(across(starts_with("date."), ~ as.numeric(.x))) %>%
...
df_cleaned <- df_cleaned %>%
  mutate(
    ...
    final.fate = case_when(!is.na(jdate.exit) & is.na(final.fate) ~ "pupa",
                                TRUE ~ as.character(final.fate)),
    # fix dead pupa/late culls
...
df_cleaned <- df_cleaned %>% mutate(
  if.stuck = case_when(is.na(jdate.stuck) ~ "N", 
                       TRUE ~ "Y"),
  if.pupa = case_when(is.na(jdate.LPI) & is.na(jdate.pupa) ~ "N", 
                      TRUE ~ "Y"),
  if.sup = case_when(is.na(jdate.6th) ~ "N", 
                     TRUE ~ "Y"),
  # type of sup
  sup = case_when(!is.na(jdate.7th) ~ 7, 
                  !is.na(jdate.6th) ~ 6, 
                  !is.na(jdate.pupa) | !is.na(jdate.LPI) ~ 0) # TODO: check... (used to be "TRUE ~ 0")
  )
...
df_cleaned <- df_cleaned %>%
  mutate(trt.meanT = case_when(trt %in% c(260, 267) ~ 26,
                           trt %in% c(337, 330) ~ 33,
                           trt == 433 ~ 36.5,
                           trt == 419 ~ 29.5),
         trt.flucT = case_when(trt %in% c(260, 330) ~ 0,
                           trt %in% c(267, 337) ~ 7,
                           trt == 433 ~ 3.5,
                           trt == 419 ~ 10.5),
         trt.maxT = case_when(trt == 260 ~ 26,
                          trt %in% c(267, 330) ~ 33,
                          TRUE ~ 40),
         trt.minT = case_when(trt == 260 ~ 26,
                          trt %in% c(267, 419) ~ 19,
                          trt == 337 ~ 26,
                          trt%in% c(433, 330) ~ 33)) %>%
  mutate(trt.flucT = case_when(trt == 260 & 
                              expt.group %in% LETTERS[3:8] ~ 2.5,
                           TRUE ~ as.numeric(trt.flucT)))
```
...
# raw data
write.csv(df_all, file = paste0(here("data/raw/2023/"), "dev_", today, ".csv"), row.names = FALSE) 
...
# 
# write.csv(export, file = here("data/clean/2023/MQs.csv"), row.names = FALSE)
...
# 
# write.csv(export, file = here("data/clean/2023-ntw-notes.csv"), row.names = FALSE)
  
# cleaned up sexta
export <- df_cleaned %>%
  #filter(is.na(species)) %>% 
  select(-c("from.E", "location", starts_with("h."), "time.in.approx", "dv", "pupa.deformities", "mass.died", "toss.if", "old.date.pmd", "reason.pmd", "fate.code", "notes", "mass.stuck", "instar.culled", "time.culled", "mass.culled", "eclose-3"))

write.csv(export, file = here("data/clean/2023/dev.csv"), row.names = FALSE)
```
