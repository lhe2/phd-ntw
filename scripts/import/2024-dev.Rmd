...
here::i_am("scripts/import/2024-dev.Rmd")
library(here)
```
...
```{r}
df_import <- read.csv(here("data/gsheets/2024/dev.csv"), header = TRUE)
```
...
df_raw <- df_import %>%
  ...
  mutate(
    across(-starts_with("notes"), ~ gsub(.x, pattern="\\?", replacement = "")), 
    # removes `?`s from all columns excepts notes column (assuming its the last col in the df)
    across(notes, ~ gsub(.x, pattern=",", replacement = ";")),
    ...
    across(date.pmd, ~ gsub(.x, pattern = "\\s\\w*", replacement = "")),
    across(c(starts_with(c("date.", "stuck.")), "notes.lastevent"),  
           ~gsub(.x, pattern = "(\\d*/\\d*)", replacement = "\\1/24"))
...
df_cleaned <- df_filtered %>%
  mutate(
    across(starts_with(c("date.", "stuck.")),  ~as.Date(.x, format = "%m/%d/%y")),
    across(starts_with(c("mass.", "treatment", "id", "date")), ~ as.numeric(.x)),
...
# raw data
write_csv(df_raw, file = paste0(here("data/raw/2024/"), "dev_", today, ".csv"))
  
# MQs only
# export <- df_cleaned %>% filter(species == "MQ")
# 
# write.csv(export, file = paste0(here("data/clean/2024/"), "dev-MQ.csv"), row.names = FALSE)
...
export <- df_cleaned %>%
  ...
  select(c("species", "cohort", "pop", "diet", "trt", "id", # identifying info
...
write.csv(export, file = paste0(here("data/clean/2024/"), "dev.csv"), row.names = FALSE)
