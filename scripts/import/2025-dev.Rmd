...
here::i_am("scripts/import/2025-dev.Rmd")
library(here)
```
...
```{r}
raw <- read.csv(here("data/gsheets/2025/dev.csv"), header = TRUE)
...
note: use `cage` values from the tent csvs instead

```{r}
# standardise column formats
data <- data %>%
  mutate(across(starts_with("date"), ~ as.Date(.x, format = "%m/%d/%Y")),
         across(c("id", starts_with(c("mass", "date"))), ~ as.numeric(.x))) %>% 
...
write.csv(raw, file = paste0(here("data/raw/2025/"), "dev_", today, ".csv"), row.names = FALSE)
write.csv(export, file = paste0(here("data/clean/2025/"), "dev.csv"), row.names = FALSE)
```
