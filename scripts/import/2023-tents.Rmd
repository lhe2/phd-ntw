...
here::i_am("scripts/import/2023-tents.Rmd")
library(here)
```
  ...
  pairs = dfs_raw[["pairs"]] %>%
    select(-c("sex_A", "sex_B", "id_F2", "id_M2", "id_A-orig", "id_B-orig", "combo2")) %>%
      rename(trt.pair = combo,
         ...
         date.m.ec = eclose_M,
         track.m = purpose_B) %>%
    filter(!(id.f == 244 & is.na(id.m))) %>% # started solo but later paired
    drop_na(id.tent),
...
dfs_cleaned <- 
  dfs_cleaned %>% lapply(\(x){
    x %>%
      mutate(
        across(starts_with("date"),  ~ as.Date(.x, format = "%m/%d/%y")),
        across(starts_with(c("date", "n.")), ~ as.numeric(.x))) %>%
...
# add in more trt info
  mutate(ctrl = case_when(grepl("col", id) ~ "col",
         ...
         if.fridge = case_when(#jdate.paired != jdate.ec ~ "x",
                               ...
                               jdate.paired > jdate.ec | 
                                 !is.na(jdate.paired) & is.na(jdate.ec) ~ 1)
         ) %>%
