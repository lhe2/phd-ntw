...
```{r}
# jk dont actually need this

#wide_all$treatment <- as.factor()
```
...
## - 260 diurnal vs other 26 C (check ctrls)
```{r}

check_ctrls <- wide_all %>%
  filter(treatment == 260)

# working on this rn lol

mod_checkctrls <- lm(mass.pupa ~ expt.group + sex, data = check_ctrls)
  # not sure if sex should be kept in.. including it makes grp G signif

anova(mod_checkctrls)

summary(mod_checkctrls)
