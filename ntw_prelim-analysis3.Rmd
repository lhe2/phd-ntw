...
NT_labels = c("260-hatch"="26/26 (26±0°C)", "267-hatch"="33/19 (26±7°C)","419-hatch"="40/19 (29.5±10.5°C)", "337-hatch"="40/26 (33±7°C)", "433-hatch"="40/33 (36.5±3.5°C)")
...
## - filter experiment groups
```{r}
# easier separation of trt groups from total data set
...
# 1.0 overall growth trends

compare overall development of all individuals compared to average growth

## set up the df
```{r}
# pull out the normal lab + field bugs
wide_lf <- wide_all %>% filter(pop!="F1") %>% filter(pop!="ctrl")
long_lf <- long_all %>% filter(pop != "F1")

# individual dev stats
long_timetopup <- wide_lf %>% 
  filter(final.fate != "pmd")


long_masstopup <- long_lf %>%
  filter(instar == "3rd" | instar == "4th" | instar == "5th" | instar == "6th" | instar == "7th" | instar == "wander", instar == "pupa") 

devstats_labL <- merge(long_timetopup, long_masstopup, all = T)

avgdevstats_labL <- long_lf %>%
  filter(instar == "3rd" | instar == "4th" | instar == "5th" | instar == "6th" | instar == "7th" | instar == "wander", instar == "pupa") %>%
...
# 2.0 plot end points and compare effects of trt
