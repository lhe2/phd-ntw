...
## - checking time to pup
```{r}
mod_checktt_pup <- lm(jdate.pupa ~ expt.group + sex + pop + treatment, data = wide_all)
...
check_ttpup_lab <- wide_all %>% filter(pop == "lab")

mod_checkttpup_lab <- lm(jdate.pupa ~ expt.group + sex + treatment, data = check_ttpup_lab)

anova(mod_checkttpup_lab)

summary(mod_checkttpup_lab)



check_ttpup_field <- wide_all %>% filter(pop == "field")

mod_checkttpup_field <- lm(jdate.pupa ~ expt.group + sex + treatment, data = check_ttpup_field)

anova(mod_checkttpup_field)

summary(mod_checkttpup_field)
