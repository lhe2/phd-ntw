...
summ_hatchmod <- data_tstats %>%
  ...
  mutate(n.tothatch.overall = sum(n.tothatch),
         ...
         lt.hrate = log(rate.hatch + 1)
         ) %>%
  filter(n.tothatch.overall > 0)
  
summ_hatchall <- summ_hatchmod # to include hatch rate = 0
...
```
they look okay, i guess? 😬 nothing looks horribly wrong...
...
- AIC = 876.3347 → so this is a marginally better model for the `rate.overall` data
...
**summary:** 
- am i doing this right.. (should i be using `lm()` for this?) → even after log-transforming, there's still non-ints so can't use poisson...
...
```{r}
# ignore: this is using the wrong predictors (tent cant be a fixed eff since tent 1 in trt 1 ≠ tent 1 in trt 2)
...
- AIC: 1006.285 → so this is deffo a worse model. exclude this in the distributions checking
...
summ_hatch_plot <- data_tstatsL %>%
  ...
  mutate(trt.win = case_when(trt.f == trt.m & trt.f == 260 ~ "26-26",
         ...
         trt.sex = case_when(trt.f == trt.m ~ "F + M",
...
p1 + p2 +
  ...
  labs(shape = "treated sex", color = "temperature treatment")
```
...
(reusing the models from above ... (i.e try 3 ish)) → which models to look at for in particular?
...
summ_longevity %>%
  mutate(labels.alltrts2 = case_when(trt == 260 ~ "control",
                         ...
                         trt == 426 ~ "26°C",
                        trt == 433 ~ "33°C"),
         labels.alltrts2 = factor(labels.alltrts2, levels = c("control", "19°C", "26°C", "33°C"))) %>%
  #filter(labels.alltrts2 != "control") %>%
  ggplot(aes(y = avg.surv, x = labels.alltrts2, #color = as.factor(trt), 
             shape = sex)) +
  geom_point(size = 2, position = pd) +
  geom_errorbar(aes(ymax = avg.surv + se.surv, ymin = avg.surv - se.surv,
                    width = 0.5),
...
# model hatch rate
mod.hrate <- lm(lt.hrate ~ as.factor(trt.f) + as.factor(trt.m) + rate.collf, data = summ_hatchLB)
anova(mod.hrate, test = "Chisq")
