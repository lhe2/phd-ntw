...
# avg mass
summary_NTs %>% ggplot(aes(y = avg.mass, x = meanT, color = if.fluct)) +
...
# avg tt pup
summary_NTs %>% ggplot(aes(y = avg.tt, x = meanT, color = if.fluct)) +
...
# survival
summary_NTs %>% ggplot(aes(y = prop.survpup, x = meanT, color = if.fluct)) +
  geom_point() +
  facet_wrap(~pop) + theme_bw() +
  labs(title = "NTs: survival to pupation", 
       y = "proportion survived to pupation", 
...
ggsurvplot(fit = kmfit.byflucT, data = NTs_all,
           ...
           xlab = "time in days")
  # sigh. lots to think about here.

```

overall i think this is all saying that pop/fluct/min/mean(?) all have an effect! still not quite sure how to parse out "parameterising" temperature tho... (define it by fluct only? min T? meanT? etc?)
