...
# MQs only
data_all %>% 
  filter(species == "MQ") %>% write.csv(file = "~/Documents/repos/_private/data/ntw_data/ntw_MQ_data.csv", row.names = FALSE)

# wordier notes
data_all %>% select(c("treatment", "ID", "expt.group", "location", "date.collected", "date.stuck", "time.in.approx", "dv", "mass.died", "pupa.deformities", "notes", "reason.pmd", "reason.ignore", ends_with(".stuck"), ends_with(".culled"))) %>%
  write.csv(file = "~/Documents/repos/_private/data/ntw_data/ntw_notes.csv", row.names = FALSE)
...
write.csv(check_final, file = "~/Documents/repos/_private/data/ntw_data/clean-gsheets.csv", row.names = FALSE)
```
