...
(this mostly takes the code form `overall_controls.Rmd` LOL)
...
summary_temps <- temps_all %>%
  ...
  # # nvm lets NOT do this bc i'll forget when we do the surv stuff
```
...
summary_temps %>% ggplot(aes(y = avg.mass, x = avg.tt, color = as.factor(meanT), shape = as.factor(flucT))) +
  ...
  scale_color_manual(labels = c("26°C", "33°C"), values = c("#00BFC4", "#F8766D")) +
...
(as a followup to km plot -> model w/ glms?)
