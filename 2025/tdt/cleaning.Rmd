...
```{r}
# editing time data for some bugs bc otherwise things are messed up:
  # recovery time for the day 2 pm R2 bugs
  # hatch time for day 3 R3 bugs
data <- data %>%
...
data <- data %>%
  ...
  # consolidate died/culled into "exit"
...
write.csv(raw, file = paste0("~/Documents/repos/ntw/2025/tdt/data/archive/", today, "_raw.csv"), row.names = FALSE)
write.csv(data, file = paste0("~/Documents/repos/ntw/2025/tdt/data/archive/", today, "_clean-full.csv"), row.names = FALSE)
...
write.csv(data, file = paste0(here(bin_paths$y25$tdt-data), "/clean-working.csv"), row.names = FALSE)
```
