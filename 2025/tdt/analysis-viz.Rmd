...
#source(here(bin_paths$y25$tdt, "analysis-utils_wip.R"))
```
...
dfs <- bin_wrangled$km # cohorts B+ for R2+

# default dfs$group that can be overwritten
dfs <- list_assign(dfs, 
                   group = calc.kmtimes(dfs$data, cohort, trt.recover, trt.duration))
```
...
### ctrl kms
```{r}
# plot ctrls by cohort, strata by shelf
  # lmaoooo

obj <- Surv(dfs$subsets$ctrl40$exit.fromenter, dfs$subsets$ctrl40$km.status)
fit <- survfit(obj ~ shelf, data = dfs$subsets$ctrl40)
dfs$group <- calc.kmtimes(dfs$subsets$ctrl40)


ggsurvplot(fit, data = dfs$subsets$ctrl40,
           #color = "strata",  # this doesnt play nice w scale_color_manual
           conf.int = TRUE,
           facet.by = "cohort",
           ) %++%
  #scale_color_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005"))  %++%  
  #scale_fill_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005")) +  
  labs(caption = "40C controls; time = hrs from enter") + 
  viz.kmtimes(dfs$group)
...
```{r}
# 24h
obj <- Surv(dfs$subsets40$rec24$exit.fromreturn, dfs$subsets40$rec24$km.status)
fit <- survfit(obj ~ trt.recover, data = dfs$subsets40$rec24)
dfs$group <- calc.kmtimes(dfs$subsets40$rec24) %>% calc.kmtimes0(day.return)


ggsurvplot(fit, data = dfs$subsets40$rec24,
           #conf.int = TRUE
           ) %++%
  scale_color_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005"))  %++%  
  scale_fill_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005")) +  
  labs(caption = "24h recovery; time = hrs from return") +
  viz.kmtimes(dfs$group)
...
# 48h
obj <- Surv(dfs$subsets40$rec48$exit.fromreturn, dfs$subsets40$rec48$km.status)
fit <- survfit(obj ~ trt.recover, data = dfs$subsets40$rec48)
dfs$group <- calc.kmtimes(dfs$subsets40$rec48) %>% calc.kmtimes0(day.return)

ggsurvplot(fit, data = dfs$subsets40$rec48,
           #conf.int= TRUE
           ) %++%
  scale_color_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005"))  %++%   
  scale_fill_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005")) +  
  labs(caption = "48h recovery; time = hrs from return") +
  viz.kmtimes(dfs$group)
```
...
```{r}
obj <- Surv(dfs$subsets$ctrl40$exit.fromenter, dfs$subsets$ctrl40$km.status)
fit <- survfit(obj ~ cohort, data = dfs$subsets$ctrl40)
dfs$group <- calc.kmtimes(dfs$subsets$ctrl40)
...
obj <- Surv(dfs$subsets40$all$exit.fromreturn, dfs$subsets40$all$km.status)
fit <- survfit(obj ~ trt.recover + trt.duration, data = dfs$subsets40$all)
dfs$group <- calc.kmtimes(dfs$subsets40$all) %>% calc.kmtimes0(day.return)

ggsurvplot(fit, data = dfs$subsets40$all,
           #conf.int= TRUE,
           linetype = c(2, 3, 2, 3, 2, 3, 1),
           ## show strata names/order
           # risk.table = TRUE,
           # risk.table.col = "strata",
           # risk.table.height = 0.5,
           ) %++%
  scale_color_manual(values = c("#4B99E9", "#4B99E9","#6FCF59", "#6FCF59", "#E7BF27", "#E7BF27","#E97005"))  %++%   
  #scale_fill_manual(values = c("#4B99E9", "#4B99E9","#6FCF59", "#6FCF59", "#E7BF27", "#E7BF27","#E97005")) %++%
  labs(caption = "24 and 48h recovery; time = hrs from return") +
  viz.kmtimes(dfs$group)
```
...
```{r}
obj <- Surv(dfs$subsets40$rec24$exit.fromenter, dfs$subsets40$rec24$km.status)
fit <- survfit(obj ~ trt.recover, data = dfs$subsets40$rec24)
dfs$group <- calc.kmtimes(dfs$subsets40$rec24)
...
ggsurvplot(fit, data = dfs$subsets40$rec24,
  ...
  scale_color_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005"))  %++%  
  scale_fill_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005")) +  
  labs(caption = "24h recovery; time = hrs from enter") +
  viz.kmtimes(dfs$group)


obj <- Surv(dfs$subsets40$rec48$exit.fromenter, dfs$subsets40$rec48$km.status)
fit <- survfit(obj ~ trt.recover, data = dfs$subsets40$rec48)
dfs$group <- calc.kmtimes(dfs$subsets40$rec48)
...
ggsurvplot(fit, data = dfs$subsets40$rec48,
  ...
  scale_color_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005"))  %++%   
  scale_fill_manual(values = c("#4B99E9", "#6FCF59", "#E7BF27", "#E97005")) +  
  labs(caption = "48h recovery; time = hrs from enter") +
  viz.kmtimes(dfs$group)
```
```{r}
obj <- Surv(dfs$subsets$at40$exit.fromenter, dfs$subsets$at40$km.status)
fit <- survfit(obj ~ trt.recover + trt.duration, data = dfs$subsets$at40)
dfs$group <- calc.kmtimes(dfs$subsets$at40)

ggsurvplot(fit, data = dfs$subsets$at40,
           #conf.int= TRUE,
           linetype = c(2, 3, 2, 3, 2, 3, 1),
           ## show strata names/order
           # risk.table = TRUE,
  ...
  scale_color_manual(values = c("#4B99E9", "#4B99E9","#6FCF59", "#6FCF59", "#E7BF27", "#E7BF27","#E97005"))  %++%   
  #scale_fill_manual(values = c("#4B99E9", "#4B99E9","#6FCF59", "#6FCF59", "#E7BF27", "#E7BF27","#E97005")) %++%
  labs(caption = "24 vs 48h recovery; time = hrs from enter") +
  viz.kmtimes(dfs$group)
```
### surv props
...
dfs$r1 <- list_assign(dfs$r1, survprop1 = calc.surv_ssC(dfs$subsets$at40, cohort, trt.duration, trt.recover))
```

```{r}
## maybe do mean+/-se deaths if doing across cohorts or sth... (instead of each cohort individually)
...
dfs$r1$survprop1 %>%
  ...
  ggplot(aes(x = timept, y = P, 
             color = trt.recover,
             lty = cohort,
             group = interaction(cohort, trt.recover, trt.duration)
         ))+
  geom_point() +
  geom_line() +
  #geom_point(aes(interaction = c("prop", "timept"))) +
  facet_grid(#trt.recover
             ~trt.duration, scales = "free") +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  labs(y = "proportion survived (cumulative)", # check filter
       caption = "R2+3")
...
dfs$r1$survprop1 %>%
  ...
  facet_grid(#trt.recover
             ~trt.duration, scales = "free") +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  labs(y = "proportion survived (per timept)", # check filter
       caption = "R2+3")
```
...
ggsurvplot(fit, data = dfs$data,
  ...
  geom_vline(data = dfs$group, aes(xintercept = day.recover), color = "skyblue2") +
...
dfs$r1 <- list_assign(dfs$r1, 
                      survprop1a = calc.surv_ssB2(dfs$data, trt, trt.duration, trt.recover),
                      survprop1b = calc.surv_ssB2(dfs$data, cohort, trt, trt.duration, trt.recover)
                      )
```
...
```{r}
dfs$r1 <- list_assign(dfs$r1, 
                      survprop2 = calc.surv_ssB2(dfs$r1$allB, trt, trt.duration, trt.recover),
                      survprop2a =  calc.surv_ssB2(dfs$r1$allB, trt, trt.duration, trt.recover),
                      )
```
...
```{r}
dfs$r1 <- list_assign(dfs$r1, survprop = calc.surv_ssB(dfs$r1$allB, trt, trt.duration, trt.recover))
```
