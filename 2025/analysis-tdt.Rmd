...
(as of 2025-07-16 this is really just a bunch of EDA)
...
```{r}
# expt + ctrls
obj <- Surv(dfs$all$exit.fromenter, dfs$all$km.status)
fit <- survfit(obj ~ trt.recover + trt.duration, data = dfs$all)

ggsurvplot(fit, data = dfs$all,
...
fit <- survfit(obj ~ trt.recover + trt.duration, data = km$data)
```
...
2025-07-16: if going to do these analyses, filter on cohort A first
