...
data <- data %>%
  ...
  mutate(dh.hatch = as_datetime(paste(date.hatch, time.hatch), format = "%Y/%m/%d %H:%M"),
         ...
         dh.recover = as_datetime(paste(date.return, time.recover), format = "%Y/%m/%d %H:%M"),
...
data <- data %>%
  select(c("cohort", "shelf", starts_with("trt"), "id", starts_with(c("dh", "jdate", "mass")), "instar.exit", "fate"))
