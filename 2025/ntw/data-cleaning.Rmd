...
dfs_filtered <- list(
  ad23 = dfs_imported[["ad23"]] %>%
    select(c("cage", starts_with("jdate"), ends_with(c(".m", ".f"))),
           -starts_with("track")),
...
dfs_filtered$ad23 <- dfs_filtered$ad23 %>%
    mutate(id.m = case_when(id.m %in% c("col", "ctrl") ~ paste0(id.m, "-m", sprintf("%03s", rownames(.))),
                            #id.f == "146" & id.m == "253" ~ NA_character_,
                            TRUE ~ as.character(id.m)),
           id.f = case_when(id.f %in% c("col", "ctrl") ~ paste0(id.f, "-f", sprintf("%03s", rownames(.))),
