...
# needs to be "wide" (...f, ...m) to determine mate types
...
# to determine m/f counts for cage events
...
dfs_tents <- list(
  ...
  info = dfs_imported[["ad25"]] %>%
    pivot_wider(id_cols = c("id", "cage"), 
                names_from = "sex", values_from = "pop") %>%
    drop_na(cage) %>% #View()
    rename(pop.f = f, pop.m = m) %>% #View() #for dfs23
