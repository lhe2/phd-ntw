...
.CalcTentsSS <- function(x){
  ...
  summarise(
    ...
    n.viable.total = sum(eggs.fert),
    n.coll.total = sum(eggs.coll),
    n.hatch.total = sum(eggs.hatched),
    n.coll.perf = n.coll.total/n.f.total,
  ...
  summarise(
    ...
    avg.coll.total = mean(n.coll.total),
    se.coll.total = se(n.coll.total),
    ...
    p.hatch = sum(n.hatch.total)/sum(n.viable.total), 
    se.hatch = se.prop(p.hatch, sum(n.viable.total))
            ) %>%
  #filter(mate.type == "within", mate.pop == "lab", mate.trt == 426) %>%
