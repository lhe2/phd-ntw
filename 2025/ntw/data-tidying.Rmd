...
```{r load larval data}
dfs_imported <- list(
  y23 = read.csv(here(bin_paths$y23$data, "clean-ntw.csv"), header = TRUE) %>%
    mutate(year = 2023) %>%
    mutate(ID = case_when(ID == 1379 & !is.na(jdate.pmd) ~ 1379.1,
                          TRUE ~ as.numeric(ID))),
  y24 = read.csv(here(bin_paths$y24$data, "ntw.csv"), header = TRUE) %>%
    mutate(year = 2024),
  y25 = read.csv(here(bin_paths$y25$ntwdata, "clean-larva.csv"), header = TRUE) %>%
    mutate(year = 2025)
)
```
