...
.GenerateCageMateInfo <- function(x){
  x %>%
    mutate(mate.type = case_when(trt.f == trt.m ~ "within",
           ...
           mate.pair = case_when(trt.f == trt.m ~ "M + F same trt",
                                 trt.f > trt.m ~ "hot F + ctrl M",
                                 trt.f < trt.m ~ "hot M + ctrl F"),
...
```{r export for stats, eval=FALSE, include=FALSE, purl=FALSE}
# update objects in environment before saving RDS!
list(wide = dfs$wide,
     long = dfs$long) %>% 
  saveRDS(., file = here(bin_paths$y25$ntwdata, "dev_tidy.rds"))

dfs$tents %>%
  saveRDS(., file = here(bin_paths$y25$ntwdata, "tents_tidy.rds"))
```
