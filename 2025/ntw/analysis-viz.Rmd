...
dfs_dev_ss <- 
  readRDS(here(bin_paths$y25$ntwdata, "dev_ss.rds")) %>%
    lapply(., \(x){
    x %>%
      ungroup() %>%
...
dfs_tents_ss <-
  readRDS(here(bin_paths$y25$ntwdata, "tents_ss.rds")) %>%
    lapply(., \(x){
    x %>%
      ungroup() %>%
      mutate(across(c("year", "mate.trt"), as.factor),
...
.plotfn <- function(x){
  ...
  ggplot(aes(#y = avg.coll.perf, 
             ...
             x = mate.trt,
             lty = mate.pair, shape = mate.pair,
             group = interaction(mate.pair, mate.hs),
             col = mate.pair
  ...
  labs(col = "treatment pair", lty = "treatment pair", shape = "treatment pair",
