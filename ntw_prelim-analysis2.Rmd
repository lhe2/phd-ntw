...
##### ok this needs to be revisited bc the adults are breaking lol

long_Ldevtime <- data_all %>%
...
long_Amass <- long_all %>%
  ...
  group_by(pop, sex, instar, trt.stage) %>%
  mutate(logmass = log(mass)) %>%
  summarise(avg.mass = mean(na.omit(mass)),
            se.mass = sd(na.omit(mass))/sqrt(length(na.omit(mass))),
            avg.logmass = mean(na.omit(logmass)),
...
long_Ldev <- merge(long_devtime, long_Lmass, all = T) %>% filter(pop!="F1") %>% filter(pop!="ctrl")
long_Adev <- merge(long_devtime, long_Amass, all = T) %>% filter(pop!="F1") %>% filter(pop!="ctrl")
# 
...
l_devavg <- long_Ldev %>% filter(instar == "3rd" | instar == "4th" | instar == "5th" | instar == "wander")
a_devavg <- long_Adev %>% filter(instar == "pupa" | instar == "wander")
...
#tempsA_devavg <- a_devavg %>% filter.temps() %>% filter(instar == "pupa" | instar == "eclose")
...
### temp mass
temps_la %>%
  filter(pop == "lab") %>% filter(instar != "pupa" & instar != "eclose") %>%
...
# temps_la %>%
#   filter(pop == "field") %>% filter(instar != "pupa" & instar != "eclose") %>%
#   ggplot(aes(x = meanT, y = avg.mass, color = factor(flucT))) +
#   geom_line(aes(lty=factor(flucT), group=interaction(meanT, flucT))) +
#   geom_point(aes(shape = factor(flucT)), size=2.5) +
#   geom_errorbar(aes(ymin = avg.mass - se.mass, ymax = avg.mass + se.mass), width = 0.15) +
...
instar_la %>%
  ...
  geom_line(aes(group=interaction(meanT, flucT))) +
...
### NT mass
NT_la %>%
  filter(!(meanT==26.0 & flucT == 7.0)) %>%
  filter(!(instar == "pupa" | instar == "eclose" | instar == "NA")) %>%
  ...
  scale_color_manual(values = c("#018571", "#80CDC1", "#E66101", "#A6611A")) +
...
NT_la %>%
  ...
  theme_bw() +
  facet_wrap(~instar, scales = "free") +
  labs(y = "NT: time to instar (days)", color = "fluctuation") +
  scale_color_manual(values = c("#018571", "#80CDC1", "#E66101", "#A6611A")) +
  scale_shape_manual(values = c(1, 16))
...
NT_aa %>%
  ...
  facet_wrap(instar~sex, scales = "free") +
  theme_bw() +
  labs(y = "NT: mass at instar (mg)", color = "fluctuation") +
  scale_color_manual(values = c("#018571", "#80CDC1", "#E66101", "#A6611A")) +
  scale_shape_manual(values = c(1, 16))
