...
data_all <- read.csv("~/Documents/repos/_not-public/1_data/ntw_data/development_v3.csv", header = T) # changed to v3 bc of some column reorg/hard coding
# change to this bc v1 is missing some info
...
data_notes <- select(data_all, c("treatment", "
                                 ID", "expt.group", "attempt.pupa", "got.stuck", ends_with("_ex")))
...
# create julian date columns, additional column fixing and creation
  # ignore the meantT/fluctT stuff bc i hardcoded that in v3
data_all <- data_all %>% mutate(across(starts_with("date."),  as.Date, format = "%m/%d/%y")) %>%
  ...
  mutate(across(starts_with("jdate."), as.numeric),
         ...
         # meanT = ifelse(treatment == "419", 29.5, 
         #                ifelse(treatment == "433", 36.5,
         #                       ifelse((treatment=="260" | treatment=="267"), 26, 33))),
         # fluct = ifelse(treatment == "419", 10.5,
         #                ifelse(treatment == "433", 3.5,
         #                       ifelse((treatment == "260" | treatment == "330"), 0, 7)))
         )
...
data_pup <- data_all %>% filter(final.fate == "other" | final.fate == "eclose" | final.fate == "pupa")
...
#data_C <- long[long$expt.group == "C",]


### this is updated w/ v3 data but we're moving to a new sheet Rmd lol


```

```{r define factors, labels, aesthetics}
...
#C_trts = c("260-hatch", "40-19", "337-hatch", "40-33") # manually set factor levels
C_labels = c("267"="26±7°C (33/19)","419"="29.5±10.5°C (40/19)", "426"="33±7°C (40/26)", "433"="36.5±3.5°C (40/33)")
C_hex = c("#F4B942", "#4059AD", "#6B9AC4", "#97D8C4")
C_colors = c("267"="#F4B942", "419"="#4059AD", "426"="#6B9AC4", "433"="#97D8C4")
...
AB2_lmass <- long %>%
  ...
  mutate(logmass = log(mass)) %>% # 230710: took out the na.omit() to make it work. idky it broke tho
...
#AB2_lNT <- AB2_lstats %>% (trt.stage == "267-hatch" | )


```


```{r TEMP grp C analysis until they pupate}
### STATS

long2 <- data_all %>% filter(trt.stage == "267-hatch" | trt.stage == "337-hatch" | expt.group == "C") %>%
...
long2$treatment[long2$treatment==337] <- 426

C_mass <- long2 %>%
  drop_na(mass) %>%
  group_by(treatment, instar) %>%
...
C_dev <- data_all %>%
  filter(trt.stage == "267-hatch" | trt.stage == "337-hatch" | expt.group == "C")

C_dev$treatment[C_dev$treatment==337] <- 426

C_devlong <- C_dev %>%
  #drop_na(starts_with("mass")) %>%
  select(-starts_with(c("h", "date", "mass", "reason."))) %>%
  mutate("2nd" = jdate.2nd - jdate.hatch,
         "3rd" = jdate.3rd - jdate.hatch,
         "4th" = jdate.4th - jdate.hatch,
...
C_stats <- merge(C_mass, C_devlong, all = T) 
...
C_stats %>% 
  filter(treatment != 267) %>%
  ggplot(aes(x = avg.daysto, y = avg.logmass, shape=instar, color=treatment)) +
...
### STATS 2: instar growth rate (see below alt analyses)
...
sumstats_all$instar.enter <- factor(sumstats_all$instar.enter, levels=c("hatch", "3rd", "4th"))
...
sumstats_instar <- filter(sumstats_all, meanT == "33.0")
sumstats_NT <- filter(sumstats_all, (((meanT == "29.5" | meanT == "36.5") | fluct == 7.0) & instar.enter == "hatch"))
...
sumstats_ad$instar.enter <- factor(sumstats_ad$instar.enter, levels=c("hatch", "3rd", "4th"))
