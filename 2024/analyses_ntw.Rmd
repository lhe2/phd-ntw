...
# survival
surv_summary %>%
  ggplot(aes(y = prop.survpup, x = as.factor(minT), shape = trt.type, color = diet)) +
  geom_point(size = 2) +
  geom_line(aes(group = interaction(diet, trt.type))) +
  #geom_text(aes(label = n), vjust = 1, hjust = -0.5) +
  geom_text_repel(aes(label = n), show.legend = FALSE) +
  facet_wrap(~pop) +
  theme_bw() +
  labs(title = "pupal survival",
       y = "proportion survived to pupation", x = "minimum temperature (°C)",
       shape = "treatment group", color = "diet type")

# pupal mass
dev_summary %>%
  ggplot(aes(y = avg.mass, x = as.factor(minT), shape = trt.type, color = diet)) +
  geom_point(size = 2) +
...
# time to pup
dev_summary %>%
  ggplot(aes(y = avg.tt, x = as.factor(minT), shape = trt.type, color = diet)) +
  geom_point(size = 2) +
...
- notes 2024-10-03:
  ...
  - def need to exclude the lab bug diet type variation....
