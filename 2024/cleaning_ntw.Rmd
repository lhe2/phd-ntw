...
data$instar.enter[data$instar.enter == "1st"] <- "hatch" 
  # change field 1sts to hatchlings
...
data <- data %>%
  mutate(sup = case_when(!is.na(date.6th) ~ 6,
                         !is.na(date.7th) ~ 7,
                         (is.na(date.pmd) & !is.na(date.eclose)) ~ 0)) 
...
also not sure why `row.names` stopped existing lol
...
data %>%
  ...
  write_csv(., file = "~/Documents/repos/ntw/2024/data/ntw-notes.csv", #row.names = FALSE
            )
  
# MQs only
data %>%
  filter(species == "MQ") %>% 
  write_csv(file = "~/Documents/repos/ntw/2024/data/ntw-MQ.csv")
...
data %>%
  ...
  select(c("cohort", "pop", "diet", "trt", "id", # identifying info
           starts_with(c("jdate", "mass")), 
           "sex",
           ends_with("T", ignore.case = FALSE),
           "surv.outcome"
          )) %>% #View()
  write_csv(file = "~/Documents/repos/ntw/2024/data/ntw.csv")
