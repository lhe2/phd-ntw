...
list.files(path=here(out_path), pattern="-modsel\\.csv", full.names = TRUE) %>% 
  lapply(read_csv) %>% 
  purrr::reduce(merge, all = TRUE) %>% 
  write.csv(., here(out_path, "final_modelsel-results.csv"))

list.files(path=here(out_path), pattern="_anova\\.csv", full.names = TRUE) %>% 
  lapply(read_csv) %>% 
  purrr::reduce(merge, all = TRUE) %>%
  write.csv(., here(out_path, "final_anova-results.csv"))
...
dredge(glm(is.pup ~ year*pop*minT, data = ntw_mumin.dfs$nona, family = "binomial", na.action = na.fail)) %>%
  ...
  as.data.frame(.) %>% write.csv(., here(bin_paths24$y24, "figs", "feasibility", "surv_dredge-modsel.csv"))
...
mod %>%
  ...
  write.csv(here(out_path, "fitness_anova.csv"))
