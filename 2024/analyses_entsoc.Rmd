...
```{r}
# determine # of ovipositing females
summ_rate <- dtents %>%
...
negovi <- eggfix[eggfix$females.laying < 0, ]

lastovi <- eggfix[eggfix$n.curr.f > 0 & eggfix$n.ovi >= 1, ]


# then, edit values
#summ_rate$n.coll[negovi$index] <- 0
summ_rate <- summ_rate %>% 
  mutate(n.coll = case_when(rownames(summ_rate) %in% negovi$index ~ 0,
                            TRUE ~ n.coll)) %>%
  mutate(n.coll = case_when(rownames(summ_rate) %in% lastovi$index ~ n.coll + 1,
                                   TRUE ~ n.coll))

rm(cond, eggfix, negovi, lastovi)
