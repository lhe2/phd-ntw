...
# something is mega breaking and 2024 isnt showing up properly

all <- #merge(d24, d23, all = TRUE) %>%
  #data.table::rbindlist(list(d24, d23), use.names = FALSE) %>%
  bind_rows(d24, d23) %>% 
  drop_na(id) %>%
...
all <- all[grepl("^NA", rownames(all))==F,]
...
p1 <- 
  ss_wide %>% 
  ggplot(aes(y = prop.survpup, x = minT, shape = pop,
             )) +
  geom_point(size=3) + 
...
p2 <-
  ss_wide %>% 
  ggplot(aes(y = avg.tt, x = minT, shape = pop,
             )) +
...
p3 <- 
  ss_wide %>% 
  ggplot(aes(y = avg.mass, x = minT, shape = pop,
             )) +
  geom_point(size = 3) +  
