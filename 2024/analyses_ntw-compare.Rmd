  ...
  mutate(
        ...
        #                         TRUE ~ NA_integer_),
         ...
         for.ntw = case_when(year == 2023 & cohort %in% c("A", "B") ~ "N",
                             TRUE ~ "Y"),
         for.const = case_when(trt.type == "ctrl" | (meanT == 33 & flucT %in% c(0, 7)) ~ "Y",
                               TRUE ~ "N"))
...
```{r dev stats}
# create some functions so that we can calc dev stats and then stitch it all together later
...
join.devstats <- function(data){
  dev <- full_join(data[[1]], data[[2]])
  dev <- full_join(dev, data[[3]])
  dev <- full_join(dev, data[[4]])
  
  dev$stage <- factor(dev$stage, levels = c("la", "pu", "ad"))
...
ss_ntw <- lapply(ss_all[[1]], join.devstats) # doesnt work
ss_ntw <- do.call(join.devstats, lapply(ss_all[1], '[[', 1))
```
