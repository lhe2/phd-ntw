...
# add better names for the facet labels
summary_lab260.2 <- summary_lab260.2 %>%
...
data_lab260.2 %>% ggplot(aes(y = mass.pupa, x = expt.group, fill = diet)) +
  ...
  geom_text(aes(label = paste0("n = ", n), y = 15), check_overlap = TRUE)
...
data_lab260.2 %>% ggplot(aes(y = tt.pupa, x = expt.group, fill = diet)) +
  ...
  #y_err_tt(y_err= 0.9) + 
  theme_bw() +
  scale_fill_discrete(labels = c("lab diet", "tobacco diet")) + # cant read the font w the colors
  labs(title = "avg time to pupation for lab controls", 
       x = "expt group",
