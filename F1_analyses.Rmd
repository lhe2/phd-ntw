...
F1s <- F1s %>%
  mutate(tent = case_when(parent.tent == "301-I" | ID %in% c(1186, 1188, 1195, 1192, 1262, 1290, 1264, 1248) ~ "301-I",
         ...
         parent.trt = case_when(tent == "301-I" ~ "m/f @ 40-19C",
                                tent == "301-F" ~ "f @ 40-19C",
                                tent == "107-C" ~ "f @ 40-33C"),
         pop = case_when(pop == "lab" ~ "parent",
                         TRUE ~ as.character(pop)),
         pop = factor(F1s$pop, levels = c("parent","F1"))) %>%
  drop_na(tent)
```
...
plots.propsurv <- summary_F1s %>% 
  ...
  map(~ggplot(data = ., aes(y = prop.survpup, x = pop, color = treatment)) +
        ...
        scale_color_manual(values = c("#00BFC4", "#F8766D")) +
        theme(legend.position="none") +
        geom_text(aes(label = n.all), vjust = -1.1, hjust = -0.5) +
...
```{r merge plots}
grid.arrange(plots.mass[[1]], plots.tt[[1]], plots.propsurv[[1]], nrow = 1,
...
grid.arrange(plots.mass[[2]], plots.tt[[2]], plots.propsurv[[2]], nrow = 1,
...
grid.arrange(plots.mass[[3]], plots.tt[[3]], plots.propsurv[[3]], nrow = 1,
