...
data_pup <- data_pup %>% mutate(across(starts_with("date."),  as.Date, format = "%m/%d/%y")) %>%
  ...
  mutate(across(starts_with("jdate."), as.numeric),
         ...
         trt.stage = as.character(paste(treatment,instar.enter, sep = "-")))
...
# larva/adult mass @ instars
  # pupa/adults need to be separated from the larva bc of sex differences
lmass_AB2 <- long %>% #group_by(trt.stage, instar) %>%
    #group_by(treatment, instar.enter, instar) %>% # this also doesnt workkkk
    filter(instar == "3rd" | instar == "4th" | instar == "5th" | instar == "wander") %>%
    #naniar::replace_with_na_all(condition = ~.x == "NA") %>% #doesnt work
    #na.omit(mass) %>%
    #na.exclude(mass) %>%
    group_by(instar, trt.stage) %>%
    #mutate(mass = first(na.omit(mass))) %>% #i think this works
    ...
    summarise(avg.mass = mean(mass),
              ...
              n=n())
  # this is dropping the grp B 337-hatch 5ths for some reason (so probably the A's also then?)

amass_AB2 <- long %>% group_by(trt.stage, sex, instar) %>%
    filter(instar == "pupa" | instar == "adult") %>%
    mutate(log.mass = log(mass)) %>%
    rename(instar.mass = instar) %>%
    summarise(avg.logmass = mean(log.mass),
              se.logmass = sd(log.mass)/sqrt(length(log.mass)))

# calculate development time (instar duration), split into larva/adults
devtime_AB2 <- data_pup %>% 
  #drop_na(starts_with("jdate.")) %>%
  select(-starts_with(c("h", "date", "mass", "reason."))) %>%
  mutate("2nd" = jdate.2nd - jdate.hatch,
         "3rd" = jdate.3rd - jdate.hatch,
         "4th" = jdate.4th - jdate.hatch,
         ...
         #eclose = jdate.eclose - jdate.15
         adult = jdate.eclose - jdate.pupa) %>%
  select(-starts_with("jdate.")) %>%
  pivot_longer (cols = c("2nd", "3rd", "4th", "5th", "wander", "pupa", "adult"), # pivot differences by stage
                names_to = "stage",
                values_to = "days.to") %>%
  group_by(trt.stage, stage) %>%
  mutate(se.daysto = sd(days.to)/sqrt(length(days.to)),
         avg.daysto = mean(days.to))
# running into same issue here with jdate

# this looks right
ldev_AB2 <- devtime_AB2 %>% filter(stage == "3rd" | stage == "4th" | stage == "5th" | stage == "wander") %>%
  rename(instar = stage) %>%
  group_by(trt.stage, instar) %>%
  summarise(avg.daysto = mean(days.to),
            se.daysto = sd(days.to)/sqrt(length(days.to)))

adev_AB2 <- devtime_AB2 %>% filter(stage == "pupa" | stage == "adult") %>%
  rename(instar.duration = stage) %>%
  group_by(trt.stage, sex, instar.duration) %>%
  summarise(avg.daysto = mean(days.to),
            se.daysto = sd(days.to)/sqrt(length(days.to)))
...
astats_AB2 <- merge(amass_AB2, adev_AB2, all = T)


# create subsets for plotting effect of temp and temp @ instar
# temp effect subsets
AB_ltemps2 <- lstats_AB2 %>% filter(trt.stage != "337-3rd" & trt.stage != "337-4th")
AB_atemps2 <- astats_AB2 %>% filter(trt.stage != "337-3rd" & trt.stage != "337-4th")

# temp+instar effects subsets
AB_linstar2 <- lstats_AB2 %>% filter(trt.stage == "260-hatch" | trt.stage == "337-hatch" | trt.stage == "337-3rd" | trt.stage == "337-4th")
AB_ainstar2 <- astats_AB2 %>% filter(trt.stage == "260-hatch" | trt.stage == "337-hatch" | trt.stage == "337-3rd" | trt.stage == "337-4th")
...
## old A + B isolated analyses (use the pooled one)
