...
growth_AB <- long %>% group_by(trt.stage) %>%
    filter(instar == "3rd" | instar == "4th" | instar == "5th" | instar == "wander") %>%
    mutate(log.mass = log(mass))

adultmass_AB <- long %>% group_by(trt.stage, sex) %>%
  filter(instar == "pupa" | instar == "adult") %>%
  mutate(log.mass = log(mass))
...
AB_temps <- stats_AB %>% filter(treatment == "260" | treatment == "267" | treatment == "330" | (treatment == "337" & instar.enter == "hatch"))
AB_adtemps <- adultmass_AB %>% filter(treatment == "260" | treatment == "267" | treatment == "330" | (treatment == "337" & instar.enter == "hatch"))

AB_dev <- stats_AB %>% filter(trt.stage == "260-hatch" | trt.stage == "337-hatch" | trt.stage == "337-3rd" | trt.stage == "337-4th")
AB_addev <- adultmass_AB %>% filter(trt.stage == "260-hatch" | trt.stage == "337-hatch" | trt.stage == "337-3rd" | trt.stage == "337-4th")

### plots comparing effect of temps ###

# larval masses
ggplot(data = AB_temps, aes(y = log.mass)) + geom_boxplot(aes(fill = treatment)) +
  labs(title = "mass at start of instar", x = "instar", y = "log(mass (mg))") +
  scale_fill_manual(labels = A_labels, values = A_colors) + theme_bw() +
  facet_grid(~instar.mass, switch="x") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), panel.spacing = unit(0, "lines"))
...
# adult masses
AB_adtemps %>% filter(sex != "") %>% # remove the pt B wanderers for now
  ggplot(aes(x = factor(instar, level = c("pupa", "adult")), y = mass)) + geom_boxplot(aes(fill = treatment)) + facet_wrap(~sex) +
  labs(title = "adult masses", y = "mass (mg)", x = "stage") +
  scale_fill_manual(values = A_colors, labels=A_labels) + theme_bw()

# larval dev time
AB_temps %>% filter(instar.duration == "2nd" | instar.duration == "3rd" | instar.duration == "4th" | instar.duration == "5th" | instar.duration == "wander") %>%
  ggplot(aes(y = days.to)) + geom_boxplot(aes(fill = treatment)) +
  labs(title = "larval instar development times", x = "stage", y = "days after hatching") +
  scale_fill_manual(labels = A_labels, values = A_colors) + theme_bw() +
  facet_grid(~instar.duration, switch="x") +
...
# adult dev time
AB_adtemps %>% filter(instar == "pupa" | instar == "adult") %>%
  ggplot(aes(x = factor(instar, levels = c("pupa", "adult")), y = days.to)) + geom_boxplot(aes(fill = treatment)) +
  labs(title = "post-pupation development times", x = "stage", y = "days after previous stage") +
  scale_fill_manual(labels = A_labels, values = A_colors) + theme_bw()
...
# larval masses
AB_dev %>% ggplot(aes(y = log.mass)) + geom_boxplot(aes(fill = factor(trt.stage, level= B_trts))) +
  labs(title = "mass at start of instar", x = "instar", y = "log(mass (mg))", fill = "treatment") +
  scale_fill_manual(labels = B_labels, values = B_colors) + theme_bw() +
  facet_grid(~instar.mass, switch="x") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(), panel.spacing = unit(0, "lines"))
...
# adult masses
ggplot(data = AB_addev, aes(x = factor(instar, level = c("pupa", "adult")), y = mass)) +
  geom_boxplot(aes(fill = factor(trt.stage, level= B_trts))) + facet_wrap(~sex) +
  scale_fill_manual(labels = B_labels, values = B_colors) + theme_bw() +
  labs(title = "adult masses", y = "mass (mg)", x = "stage", fill = "treatment")

# larval dev
AB_dev %>% filter(instar.duration == "2nd" | instar.duration == "3rd" | instar.duration == "4th" | instar.duration == "5th" | instar.duration == "wander") %>%
  ggplot(aes(y = days.to)) + geom_boxplot(aes(fill = factor(trt.stage, level = B_trts))) +
  labs(title = "larval instar development times", x = "stage", y = "days after hatching", fill = "treatment") +
  scale_fill_manual(labels = B_labels, values = B_colors) + theme_bw() +
  facet_grid(~instar.duration, switch="x") +
...
AB_addev %>% filter(instar.duration == "pupa" | instar.duration == "pupate") %>%
