...
data_pup <- data_all %>% filter(reason.pmd == "" | reason.pmd == "pupa" | reason.pmd == "toast" ) # might need to revisit this...
...
# assign dstage (stage of death) based on when died

# coding final outcomes + addtl info
surv2 <- data_all %>% # = stage of death
  mutate(meantemp = ifelse((treatment=="260" | treatment=="267"), 26, 33),
         fluct = ifelse((treatment == "260" | treatment == "330"), 0, 7),
         dstage = case_when(reason.pmd == "empty" | reason.pmd == "squished" ~ 99, # accidental death
                            is.na(date.2nd) ~ 1, # died as 1st
                            is.na(date.3rd) ~ 2,
                            is.na(date.4th) ~ 3,
                            reason.pmd != "toast" & is.na(date.5th) ~ 4,
                            reason.pmd != "toast" & is.na(date.wander) ~ 5,
                            !is.na(date.wander) & !is.na(date.LP) ~ 6, # died as LPI
                            !is.na(date.wander) & is.na(date.LP) & is.na(date.eclose) ~ 7, # died as pupa
                            reason.pmd == "toast" ~ 8, # the toasted ones
                            TRUE ~ 0), # eclosed
         ifdied = ifelse((dstage == 0 | dstage == 8), 0, 1)) %>% # 0 = to eclosion, 1 = died
  filter(dstage != 99) # remove accidental deaths from analysis

# survival curves to eclosion
surv_ec <- surv2 %>%
  mutate(censor = case_when(dstage == 1 | dstage == 2 | dstage == 3 | dstage == 4 | dstage == 5 ~ 1,
                               dstage == 6 ~ 2, # LP
                               dstage == 7 ~ 3, # P
                               dstage == 0 | dstage == 8 ~ 0), # censor eclosion or toast
         date.exit = case_when(censor == 1 ~ date.pmd, # exit time if to eclosion
                               censor == 2 ~ date.LP,
                               censor == TRUE ~ date.eclose))
  ...
  mutate(timeto.exit = jdate.exit-jdate.hatch,
         ...
         fluct = ifelse((treatment == "260" | treatment == "330"), 0, 7),
         ifdied2 = ifelse((final.fate == "eclose" | reason.pmd == "toast"), 0, 1), # censor if they got toasted. maybe use this one
         fc2 = ifelse(final.fate=="eclose", NA, fate.code))
```
