...
data_fluct2015$from.data <- c("y2015")

# make my data look like 2015 data: remove extra columns, calculate times to instars, add surv5th and survPup,  match names
...
# mutate new columns to indicate survival like in 2015 csv
data_fluct2023 <- data_fluct2023 %>%
  filter(final.fate != "misc") %>% # remove accidental deaths from overall analysis
  mutate(surv.5th = ifelse(is.na(jdate.5th) & final.fate != "pmd", "N", "Y"),
...
data_flucts <- bind_rows(data_fluct2015, data_fluct2023) %>% select(c(1:25))
