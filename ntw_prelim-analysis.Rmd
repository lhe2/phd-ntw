...
# for some reason this broke on 230321 so is now in the setup chunk
...
# adult masses
adultmass_AB %>% filter(sex != "") %>% # remove the pt B wanderers for now
...
# adult dev time
AB_temps %>% filter(instar.duration == "eclose" | instar.duration == "pupate") %>%
...
# adult masses (data tbd)
# ggplot(data = adultmass_AB, aes(x = factor(instar, level = c("pupa", "adult")), y = mass)) + 
...
## dev time (post-pupation) (data tbd)
# AB_dev %>% filter(instar.duration == "eclose" | instar.duration == "pupate") %>%
#   ggplot(aes(x = factor(instar.duration, levels = c("pupate", "eclose")), y = days.to)) + geom_boxplot(aes(fill = treatment)) +
#   labs(title = "post-pupation development times", x = "stage", y = "days after previous stage")
