...
data_all <- read.csv("~/Documents/projects/data/ntw_data/development_v2.csv", header = T) # change to this bc v1 is missing some info
data_all <- mutate_all(data_all, na_if, "") # remove na lines introduced during import
...
surv2 <- data_all %>%
  ...
  mutate(meantemp = ifelse((treatment=="260" | treatment=="267"), 26, 33),
         fluct = ifelse((treatment == "260" | treatment == "330"), 0, 7),
         dins = case_when(is.na(date.2nd) ~ 1, # died as 1st instar
                          is.na(date.3rd) ~ 2,
                          is.na(date.4th) ~ 3,
...
surv_ec <- surv2 %>%
  mutate(dins = case_when(reason.pmd != "toast" & is.na(date.pupate) & is.na(date.LP) ~ 5, # died as 5th instar
                          !is.na(date.LP) ~ 6, # died as LPI
                          !is.na(date.pupate) & is.na(date.eclose) ~ 7, # died as pupa
...
surv_pup <- surv2 %>%
  mutate(dins = case_when(reason.pmd != "toast" & is.na(date.pupate) & is.na(date.LP) ~ 5, # died as 5th instar
         ...
         date.exit = case_when(dstage == 1 ~ date.pmd, # exit time if to pupation
                               dstage == 2 ~ date.LP,
                               !is.na(date.pupate) & is.na(date.eclose) ~ as.Date(jdate.15+10, format = "%m/%d/%y", origin = date.hatch),
                               dins == 99 ~ date.pmd,
                               dins == 0 ~ date.pupate),
         subset = "to.pup")
...
B_labels = c("260-hatch"="26°C @ hatch", "267-hatch"="26±7°C @ hatch", "330-hatch"="33°C @ hatch","337-hatch"="33±7°C @ hatch", "337-3rd"="33±7°C @ 3rd", "337-4th"="33±7°C @ 4th")
...
surv_ins <- surv3 %>% filter(trt.stage == "260-hatch" | trt.stage == "337-hatch" | trt.stage == "337-3rd" | trt.stage == "337-4th")


## create survival objects

# look @ temp surv to ec
# sobj_temp_ec <- surv_temp %>% filter(subset == "ec")
# sobj_temp_ec <-  Surv(sobj_temp_ec$timeto.exit.hatch, sobj_temp_ec$censor)
# sfit_temp_ec <- survfit(sobj_temp_ec ~ trt.stage, data = filter(surv_temp, subset == "ec"))
# 
# ggsurvplot(fit = sfit_temp_ec, data=filter(surv_temp, subset == "ec"),
#            xlab = "days from hatching",
#            ylab = "survival probability")


# can i do all 3 outcomes + facet?
sobj_temp_ec <- Surv(surv_temp$timeto.exit.hatch, surv_temp$censor)
sfit_temp_ec <- survfit(sobj_temp_ec ~ trt.stage, data = surv_temp)

ggsurvplot(fit = sfit_temp_ec, data=surv_temp,
           xlab = "days from hatching",
           ylab = "survival probability",
           facet.by = "subset",
           palette = A_colors, legend.title = "", levels = A_labels)
...
data_v1 <- read.csv("~/Documents/projects/data/ntw_data/development.csv", header = T)
