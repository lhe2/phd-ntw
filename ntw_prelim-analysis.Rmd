...
test <- filter(long, (reason.pmd == "" | reason.pmd == "toast") & (trt.stage == "337-hatch" | trt.stage == "260-hatch") & (instar == "5th" | instar == "pupa"))
#& (instar == "3rd" | instar == "4th" | instar == "5th" | instar == "wander"))
#& (instar == "5th" | instar == "wander" | instar == "pupa"))

test1 <- long %>% filter(reason.pmd == "" & trt.stage == "337-hatch" & instar == "5th") 
#%>% summarise(n = n())
...
levels(as.factor(test$expt.group))
...
lmass_AB2 <- long %>% #group_by(trt.stage, instar) %>%
    ...
    mutate(log.mass = log(mass)) %>%
    #rename(instar.mass = instar) %>%
    summarise(avgmass = mean(mass), # breaks?
              avg.logmass = mean(log.mass),
...
devtime_AB2 <- data %>% 
  ...
  mutate("2nd" = jdate.2nd - jdate.hatch,
         ...
         "5th" = jdate.5th - jdate.hatch,
         wander = jdate.wander - jdate.hatch,
         pupa = jdate.pupate - jdate.wander,
         #eclose = jdate.eclose - jdate.15
         adult = jdate.eclose - jdate.pupate) %>%
  select(-starts_with("jdate.")) %>%
