...
# for when i mess up!
ogtents <- list(data_pairs, data_tstats, data_hatch)
```
...
```{r}
test_tents <- merge(data_tstats, data_hatch, by = c("date.coll", "id.tent"), all = TRUE) 

test_hatchinfo <- select(test_tents, c(id.tent, date.hatch, n.hatch, id.coll, notes.hatch))

test_tents <- select(test_tents, -c(date.hatch, n.hatch, id.coll, notes.hatch))

test <- left_join(test_tents, test_hatchinfo, by = c("id.tent", "date.coll" = "date.hatch"), all = TRUE)
```
...
# data_tents <- left_join(data_tstats, data_hatch, by = c("date.coll" = "date.hatched", "id.tent"), all = TRUE) 
